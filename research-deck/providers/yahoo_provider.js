...
class YahooProvider extends BaseProvider {
  ...
  extractPoints() {
    ...
    if (this.doc.body) {
      ...
      // 「今すぐ利用」のポイント数を厳密に狙い撃ち (例: 今すぐ利用5.5%(64pt) または 今すぐ利用(64pt) の構造)
      const immediateMatch = text.match(/今すぐ利用(?:\d+(?:\.\d+)?%)?\(?(\d+)(?:pt|ポイント|円相当)/i);
      ...
      // 「獲得」のポイント数を厳密に狙い撃ち (例: 獲得1.5%(17pt) または 獲得(17pt) の構造)
      const afterMatch = text.match(/獲得(?:\d+(?:\.\d+)?%)?\(?(\d+)(?:pt|ポイント|円相当)/i);
      let afterPoints = 0;
      if (afterMatch) {
