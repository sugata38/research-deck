...
class YahooProvider extends BaseProvider {
  ...
  extractPoints() {
    ...
    if (this.doc.body) {
      ...
      // 「今すぐ利用」の直近（後ろ40文字以内）にあるpt付き数値を狙い撃ち
      const immediateMatch = text.match(/今すぐ利用.{0,40}?(\d+)(?:pt|ポイント|円相当)/i);
      ...
      // 「獲得」の直近（後ろ40文字以内）にあるpt付き数値を狙い撃ち
      const afterMatch = text.match(/獲得.{0,40}?(\d+)(?:pt|ポイント|円相当)/i);
      let afterPoints = 0;
      if (afterMatch) {
        ...
        for (const match of matches) {
          ...
          if (match[0].includes("%")) {
            if (val > 0 && val <= 100 && currentPrice > 0) {
              couponValues.push(Math.floor(currentPrice * (val / 100)));
            }
          } else {
